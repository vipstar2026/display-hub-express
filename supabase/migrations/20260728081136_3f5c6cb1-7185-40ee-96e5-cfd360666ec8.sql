-- Back to invoker view (avoids definer-view bypass), enforce column-level security instead
ALTER VIEW public.payment_methods_public SET (security_invoker = true);

-- Row policy: only active rows visible to visitors
...
-- Column-level grants: no credentials / config columns
