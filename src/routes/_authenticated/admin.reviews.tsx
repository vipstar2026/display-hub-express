...
function AdminReviews() {
  ...
  const toggle = async (r: Row) => {
    ...
    toast.success(r.is_approved ? "Unapproved" : "Approved");
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Approve, unapprove, or delete customer product reviews.</p>
      </div>

      <div className="grid gap-3 sm:grid-cols-4">
        <Kpi label="Total" value={stats.total} />
        <Kpi label="Pending" value={stats.pending} accent="text-amber-400" />
        <Kpi label="Approved" value={stats.approved} accent="text-emerald-400" />
        <Kpi label="Avg rating" value={stats.avg.toFixed(2)} accent="text-primary" />
      </div>

      <div className="flex flex-wrap gap-2">
        <div className="relative min-w-[220px] flex-1">
          <Search className="pointer-events-none absolute start-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-muted-foreground" />
          <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search product / email / text…" className="h-9 ps-8" />
        </div>
              ...
              <div key={r.id} className="flex flex-col gap-3 p-4 sm:flex-row sm:items-start">
                ...
                <div className="flex shrink-0 gap-1.5">
                  <Button size="sm" variant={r.is_approved ? "outline" : "default"} onClick={() => toggle(r)} className={r.is_approved ? "" : "bg-primary text-background hover:bg-primary"}>
                    {r.is_approved ? <><X className="me-1 h-3.5 w-3.5" /> Unapprove</> : <><Check className="me-1 h-3.5 w-3.5" /> Approve</>}
                  </Button>
