...
import {
  LifeBuoy, Loader2, Search, Filter, RefreshCw, Mail, Phone, User,
...
export const Route = createFileRoute("/_authenticated/admin/support")({
  ...
  head: () => ({ meta: [{ title: "الدعم الفني | VIP STAR" }] }),
});
...
const STATUS_META: Record<TicketStatus, { label: string; cls: string; icon: any }> = {
  open: { label: "مفتوح", cls: "bg-blue-500/15 text-blue-600 dark:text-blue-400 border-blue-500/30", icon: AlertCircle },
  in_progress: { label: "قيد المعالجة", cls: "bg-amber-500/15 text-amber-600 dark:text-amber-400 border-amber-500/30", icon: Clock },
  resolved: { label: "تم الحل", cls: "bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 border-emerald-500/30", icon: CheckCircle2 },
  closed: { label: "مغلق", cls: "bg-muted text-muted-foreground border-border", icon: XCircle },
};
...
function AdminSupportPage() {
  ...
  const removeTicket = async (id: string) => {
    if (!confirm("حذف هذه التذكرة نهائياً؟")) return;
  ...
  return (
    <div className="space-y-6" dir="rtl">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">إدارة جميع تذاكر العملاء والمساعدة</p>
          </div>
        </div>
      ...
      <div className="grid gap-3 grid-cols-2 md:grid-cols-5">
        {[
          { label: "الإجمالي", value: stats.total, color: "from-brand to-accent2" },
          { label: "مفتوح", value: stats.open, color: "from-blue-500 to-blue-700" },
          { label: "قيد المعالجة", value: stats.in_progress, color: "from-amber-500 to-amber-700" },
          { label: "تم الحل", value: stats.resolved, color: "from-emerald-500 to-emerald-700" },
          { label: "مغلق", value: stats.closed, color: "from-muted to-muted-foreground/40" },
        ].map((s) => (
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className={`text-[10px] px-2 py-0.5 rounded-full border ${M.cls} font-medium flex items-center gap-1`}>
                      ...
                      <div className="text-[10px] text-muted-foreground font-mono whitespace-nowrap">
                        {new Date(t.created_at).toLocaleDateString("ar-EG", { day: "2-digit", month: "short" })}
                      </div>
                    </div>
                  </button>
                );
          ...
          {!selected ? (
            ...
            <div className="space-y-4">
              <div className="flex items-start justify-between gap-2">
                <div>
                  <div className="text-[10px] uppercase tracking-widest text-muted-foreground font-mono">تذكرة #{selected.id.slice(0, 8)}</div>
              ...
              <div>
                <div className="text-[10px] uppercase tracking-widest text-muted-foreground font-mono mb-1.5">رسالة العميل</div>
              ...
              <div>
                <div className="text-[10px] uppercase tracking-widest text-muted-foreground font-mono mb-1.5">الحالة</div>
                    ...
                    return (
                      <button
                        ...
                        className={`h-9 rounded-md text-xs font-medium border flex items-center justify-center gap-1.5 transition-smooth ${
                          active ? M.cls + " ring-2 ring-offset-1 ring-offset-card ring-current" : "bg-card border-border text-muted-foreground hover:text-foreground"
                        }`}
                      >
              ...
              <div>
                <div className="text-[10px] uppercase tracking-widest text-muted-foreground font-mono mb-1.5">ملاحظات داخلية</div>
                <textarea
                  value={notes}
              ...
              <div className="grid grid-cols-2 gap-2 pt-2 border-t border-border">
                ...
                >
                  <Send className="w-3.5 h-3.5" /> رد بالبريد
                </a>
                <button
