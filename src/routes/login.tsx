...
export const Route = createFileRoute("/login")({
  component: LoginPage,
  head: () => ({ meta: [{ title: "Login | VIP STAR" }] }),
});
        ...
        <div className="w-full max-w-md bg-card rounded-xl shadow-card border border-border p-8">
          ...
          <form onSubmit={onSubmit} className="space-y-3">
            ...
            <div>
              ...
              <input type="password" required value={password} onChange={(e) => setPassword(e.target.value)} className="mt-1 w-full h-11 rounded-md border border-border px-3 text-sm outline-none focus:border-brand bg-background" />
            </div>
          ...
          <p className="mt-6 text-center text-sm text-muted-foreground">
            {t("auth.noAccount")} <Link to="/signup" className="text-brand font-semibold hover:underline">{t("auth.signupNow")}</Link>
          </p>
        </div>
      </main>
      <SiteFooter />
