...
export function Header() {
  ...
  const links = [
    ...
    { to: "/blog", label: lang === "en" ? "Blog" : lang === "ur" ? "بلاگ" : lang === "bn" ? "ব্লগ" : "المدونة" },
    ...
    <header className="sticky top-0 z-50 border-b border-primary/20 bg-background/80 backdrop-blur-xl">
      <div className="container mx-auto flex h-16 items-center gap-4 px-4">
        <Link to="/" className="flex items-center gap-2 font-display text-lg font-bold">
          ...
          <span className="bg-gradient-to-r from-primary to-primary/70 bg-clip-text text-transparent">VIPSTAR</span>
        </Link>

        <nav className="hidden flex-1 items-center gap-6 md:flex">
          {links.map((l) => (
            <Link key={l.to} to={l.to} className="text-sm font-medium text-foreground/80 transition hover:text-primary">
              {l.label}
            </Link>
          ))}
        </nav>
        ...
        <div className="ms-auto flex items-center gap-2">
          ...
          {userId ? (
            ...
            <Link to="/auth"><Button size="sm" className="bg-primary text-background hover:bg-primary">{t("nav.signin")}</Button></Link>
          )}
      ...
      {open && (
        <nav className="border-t border-primary/20 md:hidden">
          <div className="container mx-auto flex flex-col gap-1 px-4 py-2">
            {links.map((l) => (
              <Link key={l.to} to={l.to} className="rounded px-2 py-2 text-sm hover:bg-primary/10" onClick={() => setOpen(false)}>
                {l.label}
              </Link>
            ))}
          </div>
